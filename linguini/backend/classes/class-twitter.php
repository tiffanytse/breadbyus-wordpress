...
class gp_Twitter {
    ...
    private function getCacheLocation() {
        return $this->defaults['directory'] . '.cache-tweet';
    }
    ...
    private function oauthGetTweets($screenname, $options) {
        ...
        if (empty($key)) return array('error'=>__('Missing Consumer Key - Check Settings', 'gp'));
        if (empty($secret)) return array('error'=>__('Missing Consumer Secret - Check Settings', 'gp'));
        if (empty($token)) return array('error'=>__('Missing Access Token - Check Settings', 'gp'));
        if (empty($token_secret)) return array('error'=>__('Missing Access Token Secret - Check Settings', 'gp'));
        if (empty($screenname)) return array('error'=>__('Missing Twitter Feed Screen Name - Check Settings', 'gp'));
