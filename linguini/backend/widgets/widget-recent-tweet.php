...
class gp_Widget_Recent_Tweet extends WP_Widget {
	...
	function gp_Widget_Recent_Tweet() {
		...
		$widget_options = array(
			...
			'description'							=> __('Widget that displays recent tweet from the Twitter account. Required settings of connection to Twitter API in Appearance > Theme Options > Socials.', 'gp')
		);
		...
		$this->WP_Widget('widget_recent_tweet', __('Linguini: Recent Tweet', 'gp'), $widget_options, $control_options);
	...
	function widget($args, $instance) {
        ...
        // Get Tweets
        $tweets = gp_tweets(1, $twitter_username);
        ?>
