...
@name 			Gallery Categories Widget
...
class gp_Widget_Categories_Gallery extends WP_Widget {

	function __construct() {
		
		$widget_options = array(
			'classname'								=> 'widget_categories',
			'description'							=> __('Widget that displays the categories of gallery. Widget only appears if there are categories.', 'gp')
		);
		
		$control_options = array(
			'width'									=> 400,
			'id_base'								=> 'widget_categories_gallery'
		);
		
		$this->WP_Widget('widget_categories_gallery', __('Linguini: Gallery Categories', 'gp'), $widget_options, $control_options);
	...
	function form($instance) {
		...
		$defaults = array(
			'widget_title'							=> __('Gallery Categories', 'gp')
		);
